...
def get_coef(dfs,myindex):
    
    df = dfs.drop(myindex,axis=0)
    ...
    myb = np.linalg.det(df)
    newdf = df.drop(0,axis=0)
    mycoef = []

    for k in range(df.shape[1]):
        mycoef.append(np.linalg.det(newdf.drop(k,axis=1))*(-1)**k)

    downpart = 0
    for k in range(df.shape[1]):
        downpart += np.linalg.det(newdf.drop(k,axis=1))**2
    downpart = downpart**0.5

    coef = [item/downpart for item in mycoef]
    b = myb/downpart
    return coef, b


def main():
    initialseq = create_seq([int(item) for item in inte.loc[0]])
    target = get_mean(dfs)
    newseq = initialseq
    count = 0
    EPOCH = 10
...
if __name__ == "__main__":
    file_name = "vertex.xlsx"
    dfs = pd.read_excel(file_name, sheet_name='Sheet1',header=None)
    file_name2 = "integer.xlsx"
    inte = pd.read_excel(file_name2, sheet_name='Sheet1',header=None)

    allcoef = {}
    allb = {}

    for i in range(13):
        coef, b = get_coef(dfs,i)
        allcoef[i] = coef
        allb[i] = b
